...
export default function AiScreen() {
  ...
  return <Screen title="ToolKit AI" subtitle="Describe the job. Find the right tools." action={<Pressable accessibilityLabel="New conversation" onPress={() => setChatId(null)} className="h-11 w-11 items-center justify-center rounded-2xl bg-card"><Plus color="#292d32" size={21} /></Pressable>}>
    ...
    {chat.data?.chat.messages.map((message) => <View key={message.id} className={`gap-3 rounded-3xl p-4 ${message.role === "user" ? "ml-8 bg-primary" : "mr-4 border border-border bg-card"}`}><Text className={message.role === "user" ? "text-white" : "text-foreground"}>{message.content || message.error}</Text>{message.results?.map((tool) => <Pressable key={tool.link} disabled={!isSafeExternalUrl(tool.link)} onPress={() => void Linking.openURL(tool.link)} className="gap-1 rounded-2xl bg-input p-3"><Text className="font-bold text-foreground">{tool.name}</Text><Text className="text-sm text-muted-foreground">{tool.description}</Text><Text className="text-xs font-semibold text-primary">{tool.reason}</Text></Pressable>)}</View>)}
    ...
    <View className="flex-row items-end gap-2 rounded-3xl border border-border bg-card p-2 pl-4"><TextInput value={query} onChangeText={setQuery} multiline placeholder="Ask for a tool…" placeholderTextColor="#6f6a87" className="max-h-32 min-h-12 flex-1 py-3 text-base text-foreground" /><Pressable disabled={!query.trim() || send.isPending} onPress={() => send.mutate(query.trim())} className="h-12 w-12 items-center justify-center rounded-2xl bg-primary disabled:opacity-50">{send.isPending ? <ActivityIndicator color="white" /> : <Send color="white" size={20} />}</Pressable></View>
    {chatId ? <Pressable onPress={() => remove.mutate(chatId)} className="min-h-12 flex-row items-center justify-center gap-2"><Trash2 color="#ed4b4b" size={18} /><Text className="font-semibold text-primary">Delete conversation</Text></Pressable> : null}
  </Screen>;
}
