...
- [ ] **Milestone 1 — In progress.** `/api/v1` now covers the planned resources, the OpenAPI client is generated, correlation headers are enforced across v1, and retry-safe mobile mutations have persistent idempotency coverage with stable keys across token refresh. Remaining work is completing shared domain-service extraction and adopting generated operation types throughout every mobile service. Signed staging OTA is an ownership gate until a real EAS project ID and signing certificate replace the placeholders.
- [ ] **Milestone 2 — Implementation complete; operational acceptance pending.** Rotating hashed refresh families, one-use database-consumed realtime tickets, OAuth PKCE, verification/recovery, session revocation, biometrics, deletion/restoration notices, and cross-schema/object-storage purge tooling are implemented. Remaining acceptance is scheduling and exercising the purge against staging backups/object storage and verifying real SMTP/provider credentials.
- [ ] **Milestone 3 — In progress.** The native shell now includes reusable buttons/cards/fields/avatars/badges/skeletons/errors/empty states/action sheets, tablet navigation rail behavior, adaptive content widths, pull-to-refresh, haptics, working high-contrast themes, and denied-permission recovery links. Remaining work is full English-catalog migration, tablet split-conversation presentation, and manual focus/screen-reader/dynamic-type audits on representative devices.
- [ ] **Milestone 4 — Implementation complete; staging acceptance pending.** Public/social navigation, collection import, saved content, multi-media post editing/viewing, message attachments/presence/history, notification filters/deletion, profile QR/deep links, and legal/support screens are implemented. Remaining work is seeded staging parity testing and visual/accessibility polish on the supported device matrix.
- [ ] **Milestone 5 — In progress.** Persisted query caches, SQLite mutation queues, retry/discard UI, media/message attachments, one-use ticket storage, reconnect reconciliation, persistent API idempotency, outbox delivery, receipts, and invalid-token handling are implemented. Remaining work is database-backed API/WebSocket/worker integration coverage and a final audit that every retry-safe mutation is queued while destructive/security mutations remain online-only.
- [ ] **Milestone 6 — In progress.** Moderation console, appeals/suspensions, configurable local content policy, GlitchTip/PostHog controls, redaction, legal drafts, store-copy/data-safety worksheets, privacy-manifest aggregation, and icon/splash/adaptive/notification assets are implemented. Remaining work is owner/legal approval, final domains/identifiers/accounts, real-device security/accessibility audits, final staged screenshots, and submission of store questionnaires/declarations.
...
<!-- NEXT_MILESTONE_TO_IMPLEMENT: Finish Milestone 1 by completing shared domain-service extraction and migrating every mobile service to generated OpenAPI operation types; then run the staging-backed integration matrix. -->
