...
export function updatePrivacy(value: Partial<PrivacyPreferences>) { return api<{ preferences: PrivacyPreferences }>("/settings/privacy", { method: "PATCH", body: JSON.stringify(value) }); }
...
export async function shareDataExport() {
  const response = await fetch(`${config.webApiUrl}/settings/export`, { headers: { Authorization: `Bearer ${useSessionStore.getState().accessToken}` } });
