...
async function messageRequest<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const response = await fetch(`${config.messageServiceUrl}${path}`, { ...init, headers: { Accept: "application/json", Authorization: `Bearer ${token}`, "X-Request-ID": clientId("req"), ...(init?.body ? { "Content-Type": "application/json" } : {}) } });
...
export async function attachmentUrl(key: string) { return (await api<OperationResult<"refreshMediaUrl", { url: string }>>(`/media/url?key=${encodeURIComponent(key)}`)).url; }
...
export async function realtimeUrl() {
  const { ticket } = await api<OperationResult<"createRealtimeTicket", { ticket: string }>>("/realtime/ticket", { method: "POST" });
