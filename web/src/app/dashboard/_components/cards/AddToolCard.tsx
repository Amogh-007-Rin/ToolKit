...
export default function AddToolCard({ isOpen, onClose, onSubmit, initialTool = null }: AddToolCardProps) {
    return (
        <AnimatePresence>
            {isOpen && (
                <>
...
function ToolForm({ initialTool, onClose, onSubmit }: Omit<AddToolCardProps, "isOpen">) {
    const [name, setName] = useState(initialTool?.name ?? "");
    const [link, setLink] = useState(initialTool?.link ?? "");
    const [selectedIcon, setSelectedIcon] = useState<string>(initialTool?.icon ?? TOOL_ICON_NAMES[0]);
    ...
    return (
        <>
            <div className="part-1 w-full h-16 flex items-center justify-between px-6 border-b border-border">
                <p className="text-xl text-foreground font-semibold">{initialTool ? "Edit Tool" : "Add Tool"}</p>
                <motion.button
                    onClick={onClose}
            ...
            <div className="part-3 w-full p-6 pt-0">
                ...
                >
                    {initialTool ? <Save size={18} /> : <Plus size={18} />}
